...
enum ModelCatalog {
    static let models: [ModelEntry] = [
        .init(id: "qwen35-08", name: "Qwen 3.5", family: "Qwen", repository: "unsloth/Qwen3.5-0.8B-GGUF", summary: "Qwen 3.5 at 0.8B parameters, with image understanding.", parameters: "0.8B", minimumMemoryGB: 4, vision: true),
        .init(id: "qwen35-2", name: "Qwen 3.5", family: "Qwen", repository: "unsloth/Qwen3.5-2B-GGUF", summary: "Qwen 3.5 at 2B parameters, with image understanding.", parameters: "2B", minimumMemoryGB: 6, vision: true),
        .init(id: "qwen35-4", name: "Qwen 3.5", family: "Qwen", repository: "unsloth/Qwen3.5-4B-GGUF", summary: "Qwen 3.5 at 4B parameters. Text and images; needs a higher-memory iPhone.", parameters: "4B", minimumMemoryGB: 8, vision: true),
        .init(id: "qwen35-9", name: "Qwen 3.5", family: "Qwen", repository: "unsloth/Qwen3.5-9B-GGUF", summary: "9B vision-language model. Q4 weights alone are about 5.7 GB, plus its vision file and runtime memory.", parameters: "9B", minimumMemoryGB: 12, vision: true),
        ...
        .init(id: "gemma3-4", name: "Gemma 3", family: "Gemma", repository: "ggml-org/gemma-3-4b-it-GGUF", summary: "Google's 4B instruction model for text and images. The 1B edition is text only.", parameters: "4B", minimumMemoryGB: 8, vision: true),
        .init(id: "gemma4-e2", name: "Gemma 4", family: "Gemma", repository: "google/gemma-4-E2B-it-qat-q4_0-gguf", summary: "The E2B edition with image understanding. Needs a higher-memory iPhone.", parameters: "E2B", minimumMemoryGB: 8, preferredQuant: "Q4_0", vision: true),
        .init(id: "gemma4-e4", name: "Gemma 4 E4B", family: "Gemma", repository: "google/gemma-4-E4B-it-qat-q4_0-gguf", summary: "Google's E4B vision-language edition. E4B is an effective-size label, not total weight count: the Q4 download is about 5.2 GB, before the vision file.", parameters: "E4B", minimumMemoryGB: 12, preferredQuant: "Q4_0", vision: true),
        ...
        .init(id: "ministral3", name: "Ministral 3", family: "Mistral", repository: "unsloth/Ministral-3-3B-Instruct-2512-GGUF", summary: "Mistral's small instruction model with image understanding.", parameters: "3B", minimumMemoryGB: 6, vision: true),
        .init(id: "smolvlm-256", name: "SmolVLM", family: "SmolVLM", repository: "ggml-org/SmolVLM-256M-Instruct-GGUF", summary: "A small vision-language model for basic image descriptions. Q8 model plus F16 vision file is about 365 MB. Limited detail and reasoning at this size.", parameters: "256M", minimumMemoryGB: 4, preferredQuant: "Q8_0", vision: true),
        ...
        .init(id: "minicpm5-2", name: "MiniCPM 5-2B", family: "MiniCPM", repository: "openbmb/MiniCPM5-2B-GGUF", summary: "OpenBMB's 2B edition; about 2.5B total parameters. Official GGUF, with a 1.6 GB Q4 download.", parameters: "2.5B", minimumMemoryGB: 6),
        .init(id: "nanbeige41-3", name: "Nanbeige 4.1-3B", family: "Nanbeige", repository: "Mungert/Nanbeige4.1-3B-GGUF", summary: "Nanbeige's 4.1 edition, separate from 4.2. About 3.9B total parameters; community GGUF with a 2.6 GB Q4 download.", parameters: "3.9B", minimumMemoryGB: 8),
        ...
        .init(id: "nemotron3-nano-4", name: "Nemotron 3 Nano", family: "Nemotron", repository: "nvidia/NVIDIA-Nemotron-3-Nano-4B-GGUF", summary: "NVIDIA's hybrid 4B text model. Official 2.8 GB Q4 download. Uses NVIDIA's Nemotron Open Model License.", parameters: "4B", minimumMemoryGB: 8),
...
actor ModelHub {
    ...
    func files(in repository: String) async throws -> ([HubFile], String?) {
        let assets = try await assets(in: repository)
        return (assets.weights, assets.license)
    }
    func assets(in repository: String, revision: String? = nil) async throws -> (weights: [HubFile], projectors: [HubFile], license: String?) {
        ...
        var endpoint = URL(string: "https://huggingface.co/api/models")!.appendingPathComponent(repository)
        if let revision { endpoint = endpoint.appendingPathComponent("revision").appendingPathComponent(revision) }
        ...
        let all = detail.siblings.filter {
            let name = $0.rfilename.lowercased()
            return name.hasSuffix(".gguf") && !name.contains("-of-") && !name.contains("mtp")
        ...
        let files = all.filter { !$0.path.localizedCaseInsensitiveContains("mmproj") }
        guard !files.isEmpty else { throw PocketError.message("No single-file model GGUFs were found. Sharded weights and safetensors are not supported.") }
        return (files, all.filter { $0.path.localizedCaseInsensitiveContains("mmproj") }, detail.cardData?.license)
    }
}
