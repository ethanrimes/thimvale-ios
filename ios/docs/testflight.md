...
**Latest push wins:** the workflow has branch/ref-scoped concurrency with `cancel-in-progress: true`. A new push to `main` cancels the older running workflow and replaces an older queued run, including its pending or active release job. Canceled runs skip test-artifact uploads to save additional time; failed tests still retain diagnostics. Group related commits into one push to avoid starting builds that will immediately be superseded. Cancellation cannot undo a package that Apple has already accepted, and it does not expire existing TestFlight builds. [GitHub's concurrency behavior](https://docs.github.com/en/actions/how-tos/write-workflows/choose-when-workflows-run/control-workflow-concurrency).
