...
If the build says **Missing Compliance**, open Manage and answer Apple's encryption questions accurately. Thimvale uses HTTPS and the iOS Keychain; do not interpret “runs locally” as “uses no encryption.” Apple's documentation says OS-provided encryption is typically exempt from documentation, but the determination must include bundled libraries. This repository intentionally does not pre-answer the export declaration for you. [Apple's encryption guidance](https://developer.apple.com/documentation/security/complying-with-encryption-export-regulations).

After confirming the applicable exemption, `INFOPLIST_KEY_ITSAppUsesNonExemptEncryption: NO` can be added under the app's base settings in `project.yml`, followed by `xcodegen generate` and a commit. This avoids repeating the questionnaire for future builds where that declaration remains accurate. Do not set it merely to bypass a warning. Changes to encryption or dependencies need another review.
