...
- **Attachments:** select multiple text files, PDFs, or images with the paperclip in Chat. Files stay in that conversation, with inline citations for text and removable draft previews. Done, transcript taps, and scrolling dismiss the keyboard. See [attachments and vision](docs/attachments-and-vision.md).
...
- **Models:** 40 curated GGUF models, plus Hugging Face search and local import. Vision badges and a filter distinguish image-capable variants; matching mmproj files have their own verified download/import. Families include Gemma, Qwen, Liquid, Granite, Phi, Llama, SmolLM, SmolVLM, Mistral, MiniCPM, Nanbeige, Nemotron, AI9Stars, Ling, Falcon, and Ornith.
...
For opt-in three-model inference, prompt-cache, and retrieval experiments, see the [simulator experiment report](docs/experiments/2026-09-07.md). It includes raw generated answers and known failures, not just retrieval scores. File tests import actual Markdown, text, CSV, JSON, HTML, and PDF files from the simulator's Documents directory through the production importer. Long experiment matrices use a separate scheme and do not consume ordinary CI time.
...
- Download availability does not establish that every architecture and quantization works on every iPhone. The runtime checks templates, model/projector loading, context capacity, and known memory overcommit. Text and image input are implemented; audio, video, sharded GGUFs, and raw safetensors are not supported. Vision inference has been exercised with SmolVLM 256M and Qwen 3.5 0.8B on the CPU simulator, not every labeled variant or physical-device Metal.
