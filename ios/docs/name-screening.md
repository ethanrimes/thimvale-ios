...
The initial cosmetic rename preserved `com.ethanrimes.pocketmind` and its saved data. The owner subsequently registered **`com.ethanrimes.thimvale`** in App Store Connect and explicitly requested that identifier; the app, signing flow, and tests now use it. This creates a separate installation from the earlier simulator prototype, without automatic cross-sandbox migration. The previous installation has not been removed. Private Application Support, Keychain-service, and background-session names remain centralized in `AppIdentity`; their unchanged strings do not grant access to another bundle's sandbox or Keychain.
